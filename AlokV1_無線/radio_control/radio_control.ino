...
int line_threshold[] = {600, 450, 700, 500};//しきい値指定
...
void loop() {
  ...
  if ( Serial1.available() ) {
    ...
    Serial.print(" angle:");
    Serial.print(controller_angle);
    Serial.print(" speed:");
    Serial.print(controller_speed);
    Serial.print(" switch:");
    Serial.println(controller_switch);
  }
  //------------------------------ライン読んどこっかな------------------------------
  for (int id = 0; id <= 3; id += 1) {
  ...
  int IR_value_angle = controller_angle;
