...
const int gyro_reset_switch = 12;
...
void setup() {
  ...
  pinMode(gyro_reset_switch, INPUT);
...
void loop() {
  if (gyro_switch_count == 0) {
    gauge = 0;
    while (!digitalRead(gyro_reset_switch)) {
      gauge++;
    }
    if (gauge > 10000) {
  ...
  if (line_degree == -1 | line_degree == -2) {
    line_reaction = 0;
  }
  ...
  if (MT_go_degree != -1) {
    for (int id = 0; id <= 3; id += 1) {
      MT_IR_speed[id] = MT_last_speed(MT_degree[id], MT_go_degree);
    }

    for (int id = 0; id <= 3; id++) {
      MT_speed[id] = (MT_IR_speed[id] * 0.7) + (gyro_degree_MT_speed * 0.3);
    }
  } else {
    for (int id = 0; id <= 3; id++) {
      MT_speed[id] = gyro_degree_MT_speed * 0.8;
    }
  }
