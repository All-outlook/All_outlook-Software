...
void loop() {
 duration1 = pulseIn(IR1, HIGH, mytimeout);
 duration5 = pulseIn(IR5, HIGH, mytimeout);
 duration9 = pulseIn(IR9, HIGH, mytimeout);
 duration13 = pulseIn(IR13, HIGH, mytimeout);
...
long correct_distance;

  if(distance >=300, distance <= 700){
 correct_distance = distance / 4 - 75;
  } else {
 correct_distance = 0;
  }

 Serial.print("degree:"); Serial.print(degree); Serial.print("° / "); 
 Serial.print("distance:"); Serial.print(correct_distance); Serial.print("cm? / ");
