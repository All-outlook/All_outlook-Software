...
float gyro() {
  ...
  float hosei_degZ = degZ;
  
  if (digitalRead(gyro_switch) == 1) {

    Serial.println("osareata");
    gyro_reference = hosei_degZ;
  }

  hosei_degZ = hosei_degZ - gyro_reference;


  while (hosei_degZ > 360) {
    hosei_degZ -= 360;
  }
  while (hosei_degZ < 0) {
    hosei_degZ += 360;
  ...
  hosei_degZ = 360 - hosei_degZ;

  return hosei_degZ;

  //Serial.print("              hosei_");
  //Serial.println(hosei_degZ);
}
